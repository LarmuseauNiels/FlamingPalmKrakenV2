...
export abstract class RaidModule {
    ...
    static async AddSingleSchedulingOptionToRaid(raidId: number, timestamp: Date) {
        // Retrieve the last scheduling option for the given raid
        const lastSchedulingOption = await global.client.prisma.raidSchedulingOption.findFirst({
            where: { RaidId: raidId },
            orderBy: { ID: 'desc' },
        });

        // Extract the Option field from the last scheduling option
        let lastOption = lastSchedulingOption ? lastSchedulingOption.Option : '@'; // '@' is the ASCII character before 'A'

        // Increment the Option to the next character in the alphabet
        let nextOption = String.fromCharCode(lastOption.charCodeAt(0) + 1);

        // Create the new scheduling option
